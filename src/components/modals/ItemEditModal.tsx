...
export const ItemEditModal: React.FC<Props> = ({ isOpen, onClose, item, isNew = false }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/75 backdrop-blur-xl animate-fade-in overflow-y-auto">
      <div className="relative w-full max-w-xl glass-modal rounded-2xl sm:rounded-3xl overflow-hidden my-auto animate-slide-up max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-3.5 sm:space-y-4 overflow-y-auto flex-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 text-xs rounded-xl sm:rounded-2xl glass-input text-white focus:outline-none"
                >
                  {streams.map(s => (
                    <option key={s} value={s} className="bg-[#121215] text-white">{s}</option>
                  ))}
                  <option value="__custom" className="bg-[#121215] text-white">+ Custom Workstream...</option>
                </select>
                {formData.stream === '__custom' && (
                  <input
                    ...
                    className="mt-1.5 w-full px-3 py-1.5 text-xs rounded-xl glass-input text-white"
                  />
                )}
              </div>
              ...
              <div>
                ...
                >
                  ...
                  <option value="__custom" className="bg-[#121215] text-white">+ Custom Owner...</option>
                </select>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 text-xs rounded-xl sm:rounded-2xl glass-input text-white font-mono"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-1.5 text-xs rounded-xl sm:rounded-2xl glass-input text-white"
                >
          ...
          {/* Milestones Sections */}
          <div className="space-y-2.5 sm:space-y-3 pt-2.5 sm:pt-3 border-t border-white/[0.08]">
            <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-[#86868b] block">
              Milestones Setup
            </span>

            {/* M1 */}
            <div className="p-3 sm:p-3.5 rounded-xl sm:rounded-2xl bg-white/[0.03] border border-white/[0.06] space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-white font-mono">Milestone 1</span>
                <select
                  value={formData.milestone1.status}
                  onChange={e => handleMilestoneChange('milestone1', 'status', e.target.value)}
                  className="px-2.5 py-0.5 text-xs rounded-full glass-input text-white"
                >
                  <option value="upcoming" className="bg-[#121215]">Upcoming</option>
                  <option value="in-progress" className="bg-[#121215]">Active</option>
                  <option value="completed" className="bg-[#121215]">Done</option>
                  <option value="delayed" className="bg-[#121215]">Delayed</option>
                  <option value="not-applicable" className="bg-[#121215]">N/A</option>
                </select>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                ...
                <div>
                  <input
                    ...
                    className="w-full px-3 py-1.5 text-xs rounded-xl glass-input text-white font-mono"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="p-3 sm:p-3.5 rounded-xl sm:rounded-2xl bg-white/[0.03] border border-white/[0.06] space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-white font-mono">Milestone 2</span>
                <select
            ...
            <div className="p-3 sm:p-3.5 rounded-xl sm:rounded-2xl bg-white/[0.03] border border-white/[0.06] space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-white font-mono">Milestone 3</span>
                <select
          ...
          {/* Footer Actions */}
          <div className="flex items-center justify-between pt-3 border-t border-white/[0.08] sticky bottom-0 bg-black/60 backdrop-blur-md -mx-4 sm:-mx-6 -mb-4 sm:-mb-6 px-4 sm:px-6 py-3">
            {!isNew ? (
              <button
                type="button"
                onClick={() => {
                ...
                className="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-rose-400 hover:bg-rose-950/30 border border-rose-500/30 rounded-full transition-colors"
              >
